...
class SeaBed extends THREE.LineSegments {
  constructor() {
    const g = new THREE.PlaneGeometry(100, 100, 120, 120).rotateX(-Math.PI * 0.5).rotateY(Math.PI * 0.25);
    ToQuads(g);
...
document.addEventListener('pointerdown', e => {
  if (panel.classList.contains('open') && !panel.contains(e.target) && e.target !== renderer.domElement) {
    closePanel();
  }
});
